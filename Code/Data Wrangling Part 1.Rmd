...
```{r data wrangling}
PhoenixAZ_processed <- read.csv("./Data/Raw/phoenix_fountainhills_raw.csv", stringsAsFactors = TRUE) %>% 
  filter(NAME == "PHOENIX AIRPORT, AZ US") %>% 
  select(NAME, DATE, PRCP, PSUN, TMAX, TMIN, TSUN) %>% 
  mutate(DATE = ymd(DATE))

FountainHillsAZ_processed <- read.csv("./Data/Raw/phoenix_fountainhills_raw.csv", stringsAsFactors = TRUE) %>% 
  filter(NAME == "FOUNTAIN HILLS, AZ US") %>% 
  select(NAME, DATE, PRCP, PSUN, TMAX, TMIN, TSUN) %>% 
  mutate(DATE = ymd(DATE))

LasVegasNV_processed <- read.csv("./Data/Raw/Nevada_data_raw.csv", stringsAsFactors = TRUE) %>% 
  filter(NAME == "MCCARRAN INTERNATIONAL AIRPORT, NV US") %>% 
  select(NAME, DATE, PRCP, PSUN, TMAX, TMIN, TSUN) %>% 
  mutate(DATE = ymd(DATE))

PahrumpNV_processed <- read.csv("./Data/Raw/Nevada_data_raw.csv", stringsAsFactors = TRUE) %>% 
  filter(NAME == "PAHRUMP 4 NW, NV US") %>% 
  select(NAME, DATE, PRCP, PSUN, TMAX, TMIN, TSUN) %>% 
  mutate(DATE = ymd(DATE))

SanDiegoCA_processed <- read.csv("./Data/Raw/SanDiego_Ramona_Raw_Data.csv", stringsAsFactors = TRUE) %>% 
  filter(NAME == "SAN DIEGO INTERNATIONAL AIRPORT, CA US") %>% 
  select(NAME, DATE, PRCP, PSUN, TMAX, TMIN, TSUN) %>% 
  mutate(DATE = mdy(DATE))

RamonaCA_processed <- read.csv("./Data/Raw/SanDiego_Ramona_Raw_Data.csv", stringsAsFactors = TRUE) %>% 
